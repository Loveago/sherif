...
function PaymentCallbackContent() {
  ...
  const verifyMutation = useMutation({
    mutationFn: () =>
      apiRequest<{ status: string; amount: number }>(
        `/wallet/paystack/verify?reference=${encodeURIComponent(reference || '')}`
      ),
    onSuccess: (data) => {
      setStatus('success');
      setMessage(`Your wallet has been funded with GHS ${data.amount}.`);
      ...
      <GlassCard className="w-full max-w-md p-8 text-center">
        ...
        {status === 'success' && (
          <>
            ...
            <div className="flex gap-3">
              <Button className="flex-1" onClick={() => router.push('/wallet')}>
        ...
        {status === 'error' && (
          <>
            ...
            <p className="text-gray-400 text-sm mb-6">{message}</p>
            <div className="flex gap-3">
              <Button variant="secondary" className="flex-1" onClick={() => router.push('/wallet')}>
                Back to Wallet
              </Button>
            </div>
          </>
