          ...
          {data.products.map((product) => (
            <GlassCard key={product.id} className="p-5">
              ...
              <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="rounded-full border border-white/10 px-3 py-1 text-xs uppercase tracking-[0.22em] text-slate-300 self-start sm:self-auto">{product.name} {product.description}</div>
              </div>
            </GlassCard>
          ))}
        </div>
